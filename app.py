...
/* Quitar header y márgenes superiores */
...
/* NumberInput (estilo claro como imagen 2) */
...
# Productos (mismo ancho + placeholders)
st.write("")
...
with pC: st.button("🗑️ Eliminar último producto", on_click=remove_last_producto,
                   disabled=(len(st.session_state.productos) <= 1), use_container_width=True)
...
with cA: st.button("➕ Agregar bulto", on_click=add_row, use_container_width=True)
with cB: st.button("🧹 Vaciar tabla", on_click=clear_rows, use_container_width=True)
with cC: st.button("🗑️ Eliminar último", on_click=remove_last, disabled=(len(st.session_state.rows) <= 1), use_container_width=True)
st.markdown('</div>', unsafe_allow_html=True)
...
with m1:
    st.session_state.peso_bruto_raw = st.text_input("Peso bruto total (kg)", value=st.session_state.peso_bruto_raw, help="Usá punto o coma para decimales (ej: 1.25)")
